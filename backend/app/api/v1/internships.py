...
async def get_internship_detail(internship_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "max_age": item.max_age,
        "skills": skill_list,
        "source": item.source or ("Jobvetta" if item.source == "Jobvetta" else ("Adzuna" if item.source == "Adzuna" else ("Lever" if item.source == "Lever" else ("Greenhouse" if item.source == "Greenhouse" else ("NCS" if "ncs.gov.in" in (item.source_url or "") else "PMIS"))))),
        "source_name": "Jobvetta Official API" if item.source == "Jobvetta" else ("Adzuna Official API" if item.source == "Adzuna" else ("Greenhouse Official" if item.source == "Greenhouse" else ("Lever Official" if item.source == "Lever" else ("National Career Service (NCS)" if (item.source == "NCS" or "ncs.gov.in" in (item.source_url or "")) else "PM Scheme Official")))),
        "opportunity_type": item.opportunity_type or "INTERNSHIP",
        "apply_url": item.apply_url or item.source_url or (f"https://www.adzuna.in/details/{item.external_id}" if item.source == "Adzuna" and item.external_id else None),
        "application_url": item.apply_url or item.source_url or (f"https://www.adzuna.in/details/{item.external_id}" if item.source == "Adzuna" and item.external_id else None),
        "external_id": item.external_id,
